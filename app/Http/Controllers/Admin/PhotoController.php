...
class PhotoController extends AdminController
{
    ...
    public function data($albumid = 0)
    {
        ...
        $photoalbum = Photo::join('language', 'language.id', '=', 'photo.language_id')
            ...
            ->select(['photo.id', DB::raw($albumid.' as albumid'), 'photo.name', 'photo_album.name as category', 'photo.album_cover', 'photo.slider',
    ...
    public function getReorder(ReorderRequest $request)
    {
        ...
        foreach ($items as $value) {
            if ($value != '') {
                Photo::where('id', '=', $value)->update(['position' => $order]);
                $order++;
            }
        }
